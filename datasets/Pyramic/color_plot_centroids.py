...
def get_intensity_centroids(I, R, N_max=50):
    I_s = I.sum(axis=0)
    max_idx = I_s.argsort()[-N_max:][::-1]
...
def draw_map(I, I_s, R, lon_ticks, catalog=None, show_labels=False, show_axis=False, idx=0):
    """
    Parameters
    ==========
    I : :py:class:`~numpy.ndarray`
    ...
    print("R_el_min, R_el_max", R_el_min, R_el_max)
    print("R_az_min, R_az_max", R_az_min, R_az_max)
    ...
    # I_mult = I_rgb.T[:, 0] + I_rgb.T[:, 1] + I_rgb.T[:, 2]
    ...
    print("K-means centers:", clusters)
    ...
    if catalog is not None:
        ...
        centroid_lon, centroid_lat = bm(clusters[:,0], clusters[:,1], inverse=True)
        print("centroid_lon, centroid_lat", centroid_lon, centroid_lat)
        centroid_lon, centroid_lat = get_intensity_centroids(I, R)
        print("[NEW - centroid_lon, centroid_lat", centroid_lon, centroid_lat)
        ax.scatter(R_x[max_idx], R_y[max_idx], c='b', s=5)
        ax.scatter(clusters[:,0], clusters[:,1], s = 500, alpha=0.1)
